...
useCase:
    ...
    - add-idp
---
# Connect Your App to Google Workplace

::: panel Using Google Social and Enterprise Connections
If you have an existing [Google Social Connection](/connections/social/google) for your application and you create a new Google Workplace connection for the same domain, users affiliated with the social connection with now be logged in with the new enterprise connection. This will occur regardless of whether you enable the Google Workplace enterprise connection.
:::
...
To connect your application to Google Workplace, you must:
...
::: panel Google Workplace Account
Before proceeding, you will need a valid Google Workplace account and must have **your own** Google Workplace Organization for which you are an administrator. 
:::

## Set up your app in Google

To allow users to log in using Google Workplace, you must register your application in the Google developer console.

::: warning
Before proceeding, you must have already set up **your own** Google Workplace Organization for which you are an administrator.
:::
...
While setting up your app, apply these settings:

* On the **OAuth consent screen**, under **Authorized domains**, add `auth0.com`.
* When asked to select an application type, choose **Web application** and set these parameters:
...
Next, you will need to create and configure a Google Workplace Enterprise Connection in Auth0. Make sure you have the **Client ID** and **Client Secret** generated when you set up your app in the Google developer console.

1. Navigate to the [Connections > Enterprise](${manage_url}/#/connections/enterprise) page in the [Auth0 Dashboard](${manage_url}/), and click the `+` next to **Google Workplace**.
...
| **G Suite Domain** | Google Workplace domain name for your organization. |
...
![Configure Advanced Google Workplace Settings](/media/articles/dashboard/connections/enterprise/conn-enterprise-gsuite-settings-2.png)

4. If you have appropriate administrative permissions to configure your Google Workplace settings so you can use Google's Admin APIs, then click **Continue**. Otherwise, provide the given URL to your administrator so that they can adjust the required settings.
